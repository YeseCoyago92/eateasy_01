...
<html>
    ...
    <body>
        ...
        /* -------------------------
          ...
          --------------------------- */
        /* Connect To Database */
        require_once ("../cliente/orden/config/db.php"); //Contiene las variables de configuracion para conectar a la base de datos
        require_once ("../cliente/orden/config/conexion.php"); //Contiene funcion que conecta a la base de datos
//        $action = (isset($_REQUEST['action']) && $_REQUEST['action'] != NULL) ? $_REQUEST['action'] : '';
//        if ($action == 'ajax') {
        // escaping, additionally removing everything that could be (html/javascript-) code
        $q = mysqli_real_escape_string($con, (strip_tags($_REQUEST['q'], ENT_QUOTES)));
        $aColumns = array('ID_ALIMENTOPREPARADO', 'NOMBRE_ALIMENTOPREPARADO'); //Columnas de busqueda
        $sTable = "tab_alimentopreparado";
        $sWhere = "";
        if ($_GET['q'] != "") {
            $sWhere = "WHERE (";
...
//                    strtoupper($res['DESCRIPCION_CATEGORIA'])
                        $id_producto = $row['ID_ALIMENTOPREPARADO'];
                        $nombre_producto = strtoupper($row['NOMBRE_ALIMENTOPREPARADO']);
                        ...
                        ?>
                        <tr>
                            <td hidden=""><?php echo $id_producto; ?></td>
                            <td><?php echo $nombre_producto . "<br>" . $descripcion_producto; ?></td>
